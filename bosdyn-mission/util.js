...
function proto_from_object(obj) {
  ...
  if (Object.prototype.toString.call(name_or_dict) === '[object Object]') {
    ...
    for (const param of name_or_dict.getParametersList()) {
      const parameter = new util_pb.VariableDeclaration().setName(param.getName()).setType(param.getType());
    ...
    for (const paramValue of name_or_dict.getParameterValuesList()) {
      ...
      if (typeof paramValue.getValue() === 'string' && paramValue.getValue()[0] === '$') {
        parameterValue.setValue(
          new util_pb.Value().setParameter(new util_pb.VariableDeclaration().setName(paramValue.getValue().slice(1))),
        );
      } else {
    ...
    for (const overrides of name_or_dict.getOverridesList()) {
      ...
      override.setValue(
        new util_pb.Value().setParameter(new util_pb.VariableDeclaration().setName(overrides.getValue())),
      );
  ...
  if ('children' in inner_proto) {
    ...
    throw new Error(
      `Proto "${node.name}" of type "${inner_type}" was given ${num_children} children,
...
class ResultFromProto {
  ...
  static proto_from_results = Object.fromEntries(
    Object.entries(ResultFromProto.results_from_proto).map(([k, v]) => [v, k]),
  );
}
...
function most_restrictive_travel_params(
  travel_params,
  vel_limit = null,
  ...
  travel_params =
    travel_params === null || travel_params === undefined ? new graph_nav_pb.TravelParams() : travel_params.clone();
