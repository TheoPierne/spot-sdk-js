...
class RemoteClient extends BaseClient {
	...
	async establish_session(leases, inputs, args){
		const req = _build_establish_session_request(inputs, leases);
		return await this.call(this._stub.establishSession, req, _session_id_from_response, _establish_error_from_response, args);
	}

	establish_session_async(leases, inputs, args){
		const req = _build_establish_session_request(inputs, leases);
		return this.call_async(this._stub.establishSession, req, _session_id_from_response, _establish_error_from_response, args);
	}

	async tick(session_id, leases, inputs, args){
		const req = _build_tick_request(inputs, leases, session_id);
		return await this.call(this._stub.tick, req, null, _tick_error_from_response, args);
	}
	...
	async stop(session_id, args){
		const req = new remote_pb.StopRequest().setSessionId(session_id);
		return await this.call(this._stub.stop, req, null, _stop_error_from_response, args);
	}
	...
	async teardown_session(session_id, args){
		const req = new remote_pb.TeardownSessionRequest().setSessionId(session_id);
		return await this.call(this._stub.teardownSession, req, null, _teardown_error_from_response, args);
	}
...
const _ESTABLISH_STATUS_TO_ERROR = {
	...
	[remote_pb.EstablishSessionResponse.Status.STATUS_MISSING_LEASES]: [MissingLeases, 'Missing leases on required resources.'],
...
const _TICK_STATUS_TO_ERROR = {
	...
	[remote_pb.TickResponse.Status.STATUS_INVALID_SESSION_ID]: [InvalidSessionId, 'Provided session ID was not valid on the server.']
}

const _STOP_STATUS_TO_ERROR = {
    [remote_pb.StopResponse.Status.STATUS_OK]: [null, null],
    [remote_pb.StopResponse.Status.STATUS_INVALID_SESSION_ID]: [InvalidSessionId, 'Provided session ID was not valid on the server.']
}

const _TEARDOWN_STATUS_TO_ERROR = {
    [remote_pb.TeardownSessionResponse.Status.STATUS_OK]: [null, null],
    [remote_pb.TeardownSessionResponse.Status.STATUS_INVALID_SESSION_ID]: [InvalidSessionId, 'Provided session ID was not valid on the server.']
}

function _establish_error_from_response(response){
    return error_factory(response, response.getStatus(), Object.keys(remote_pb.TickResponse.Status), _ESTABLISH_STATUS_TO_ERROR);
}

function _tick_error_from_response(response){
    return error_factory(response, response.getStatus(), Object.keys(remote_pb.TickResponse.Status), _TICK_STATUS_TO_ERROR);
}

function _stop_error_from_response(response){
    return error_factory(response, response.getStatus(), Object.keys(remote_pb.StopResponse.Status), _STOP_STATUS_TO_ERROR);
}

function _teardown_error_from_response(response){
    return error_factory(response, response.getStatus(), Object.keys(remote_pb.TeardownSessionResponse.Status), _TEARDOWN_STATUS_TO_ERROR);
}
