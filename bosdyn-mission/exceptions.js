  ...
  toString() {
    if (this.target_pb_type !== this.stored_pb_type) {
      return `Mismatched type for "${this.target_name}". 
            Stored as ${util.safe_pb_type_to_string(this.stored_pb_type)}, wanted ${util.safe_pb_type_to_string(
        this.target_pb_type,
      )}`;
    }
...
class MessageOverrideError extends CompileError {
  ...
  toString() {
    return `Override of type ${this.overriding_message.TypeName()} cannot be written to field "${
      this.field_name
    }" of type ${this.field_type}`;
  }
}
