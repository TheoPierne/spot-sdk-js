...
async function main(args = null){
  ...
  for(const world_obj of world_objects){
    ...
    for(const [edge] of full_snapshot.getChildToParentEdgeMapMap().entries()){
      console.log(`Child frame name: ${edge}.`,
  ...
  console.log(`Fiducial objects: \n${JSON.stringify(fiducial_objects.map(e => e.toObject()), null, 1)}`);
  ...
  console.log(`Recent objects after ${new Date(start_time).toLocaleString()} are: \n${JSON.stringify(most_recent_objects.map(e => e.toObject()), null, 1)}`);
