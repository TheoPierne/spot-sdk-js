...
```
node patch_anomalies.js --hostname ORBIT_IP --anomaly-uuid ANOMALY_UUID_1 --status open --verify False
```

Command to close a specific anomaly:

```
node patch_anomalies.js --hostname ORBIT_IP --anomaly-uuid ANOMALY_UUID_1 --status closed --verify False
```
