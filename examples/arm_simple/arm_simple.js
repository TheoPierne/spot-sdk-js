...
async function hello_arm(config){
  ...
  // an estop endpoint.
  console.assert(!(await robot.is_estopped()), "Robot is estopped. Please use an external E-Stop client, such as the estop SDK example, to configure E-Stop.");
  ...
  const odom_T_flat_body = get_a_tform_b(robot_state.getKinematicState().getTransformsSnapshot(), ODOM_FRAME_NAME, GRAV_ALIGNED_BODY_FRAME_NAME);
  ...
  let arm_command = RobotCommandBuilder.arm_pose_command(odom_T_hand.x, odom_T_hand.y, odom_T_hand.z, odom_T_hand.rot.w, odom_T_hand.rot.x, odom_T_hand.rot.y, odom_T_hand.rot.z, ODOM_FRAME_NAME, seconds);
  ...
  arm_command = RobotCommandBuilder.arm_pose_command(odom_T_hand.x, odom_T_hand.y, odom_T_hand.z, odom_T_hand.rot.w, odom_T_hand.rot.x, odom_T_hand.rot.y, odom_T_hand.rot.z, ODOM_FRAME_NAME, seconds);
...
async function block_until_arm_arrives_with_prints(robot, command_client, cmd_id){
  ...
  while (true){
    ...
    const armCartesianFeedback = feedback_resp.getFeedback().getSynchronizedFeedback().getArmCommandFeedback().getArmCartesianFeedback();
    robot.logger.info(`Distance to go: ${armCartesianFeedback.getMeasuredPosDistanceToGoal()} meters, ${armCartesianFeedback.getMeasuredRotDistanceToGoal()} radians`);

    if (armCartesianFeedback.getStatus() === arm_command_pb.ArmCartesianCommand.Feedback.Status.STATUS_TRAJECTORY_COMPLETE){
