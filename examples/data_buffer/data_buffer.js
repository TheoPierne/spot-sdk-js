...
async function add_blob(robot, options) {
  const dbuffClient = await robot.ensure_client(DataBufferClient.default_service_name);
  ...
  await dbuffClient.add_blob(msg.serializeBinary(), typename, typename, robot_timestamp, options.write_sync);
...
async function add_protobuf(robot, options) {
  ...
  await dbuffClient.add_protobuf(msg, undefined, robot_timestamp, options.write_sync);
  console.log('Added protobuf message.');
}

async function add_event(robot) {
  await (await robot.time_sync).wait_for_sync();
  await robot.log_event(
    'examples:example_event',
    data_buffer_pb.Event.Level.LEVEL_LOW,
...
async function main(args = null) {
  ...
  protobuf_parser.add_argument('--write-sync', {
    action: 'store_true',
