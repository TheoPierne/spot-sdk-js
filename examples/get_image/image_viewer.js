...
async function main(args = null) {
  ...
  while (keystroke !== VALUE_FOR_Q_KEYSTROKE && keystroke !== VALUE_FOR_ESC_KEYSTROKE) {
    try {
      const images_future = image_client.get_image_async(requests, {
        timeout: 0.5,
      });

      while (!images_future.done()) {
      ...
      const images = images_future.result();
    } catch (err) {
      if (err instanceof TimedOutError) {
        var time_err = err;

        if (timeout_count_before_reset === 5) {
          console.info('Resetting image client after 5+ timeout errors.');
