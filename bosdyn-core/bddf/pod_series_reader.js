...
class PodSeriesReader {
  ...
  read_samples(index_in_series){
    const [_desc, timestamp_nsec, data] = this._data_reader.read(this._series_index, index_in_series);
    ...
    if (data.length !== expected_size){
      throw new ParseError(`${this._series_descriptor.getSeriesIdentifier()} idx=${index_in_series} expect ${expected_size} elements but got ${data.length}`);
    }
    ...
    function _split(vals, dims){
      ...
      return range(0, vals.length, els_per_sample.length).map(x => _split(vals.slice(x, x+els_per_sample.length), next_dims));
    }
