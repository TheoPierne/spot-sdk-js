...
class FileIndexer {
  ...
  add_series(series_type, series_spec, message_type, pod_type, annotations, additional_index_names, writer){
    ...
    const series_identifier = new bddf_pb.SeriesIdentifier()
    ...
    this.add_series_descriptor(series_descriptor, series_block_file_offset);
    return series_index;
  }
  ...
  index_data_block(series_index, timestamp_nsec, file_offset, nbytes, additional_indexes){
    ...
    const block_entry = series_block_index.addBlockEntries(new bddf_pb.SeriesBlockIndex.BlockEntry().setFileOffset(file_offset));
  ...
  make_data_descriptor(series_index, timestamp_nsec, additional_indexes){
    ...
    const additional_indexes = additional_indexes || [];
    if (additional_indexes.length !== series_descriptor.getAdditionalIndexNamesList().length){
      throw new DataFormatError(`Series ${series_descriptor} needs ${series_descriptor.getAdditionalIndexNamesList().length} additional indexes, but ${additional_indexes} provided.`);
    }
    if (additional_indexes){
      for (const idx_val of additional_indexes){
