...
class StreamDataReader extends BaseDataReader {
  ...
  read_next_block(){
    ...
    if (is_data){
      this._indexer.index_data_block(desc.getSeriesIndex(), desc.getTimestamp().toDate().getTime(), data.length, file_offset, desc.getAdditionalIndexesList());
    } else{
      const desc_type = bddf_pb.SeriesDescriptor.DatatypeCase[desc.getDescriptortypeCase()];
      if (desc.hasFileIndex()) {
