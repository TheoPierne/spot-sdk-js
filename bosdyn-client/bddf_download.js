...
async function download_data(robot, hostname, start_nsec = null, end_nsec = null, timespan_spec = null, output_filename = null,
    robot_time = false, channel = null, message_type = null, grpc_service = null, show_progress = false){

    var time_sync_endpoint = null;
    if(!robot_time){
        var time_sync_client = robot.ensure_client(TimeSyncClient.default_service_name);
    ...
    if(start_nsec || end_nsec){
        var get_params = _request_timespan_from_nanoseconds(start_nsec, end_nsec, time_sync_endpoint);
    }else{
        var get_params = _request_timespan_from_spec(timespan_spec, time_sync_endpoint);
    }
    ...
    let url = _bddf_url(hostname, get_params);
    ...
    .then(res => {
        ...
        outfile = output_filename ? output_filename : _output_filename(res);
...
function _output_filename(response){
    ...
    var match = content.search('filename=\"?([^\"]+)');
...
function main(){
    ...
    var sdk = create_standard_sdk('bddf');
    var robot = sdk.create_robot(options.hostname);
    ...
    var output_filename = download_data(robot, options.hostname, options.timespan, undefined, undefined, undefined, options.robot_time, options.channel,
        options.type, options.service, true);
