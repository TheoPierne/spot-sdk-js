...
class RobotIdCommand extends Command {
  ...
  async _run(robot, options) {
    ...
    console.log(
      ` Software: ${version.getMajorVersion()}.${version.getMinorVersion()}.${version.getPatchLevel()} (${release.getChangeset()} ${timestamp_to_datetime(
...
class TextMsgCommand extends Command {
  ...
  async _run(robot, options) {
    ...
    if (options.timestamp) {
      try {
        robot_timestamp = (await robot.time_sync).robot_timestamp_from_local_secs(Date.now(), 1.0);
      } catch (err) {
...
class OperatorCommentCommand extends Command {
  ...
  async _run(robot, options) {
    ...
    if (options.timestamp) {
      ...
      try {
        await (await robot.time_sync).wait_for_sync(1.0);
      } catch (err) {
...
class DataServiceCommands extends Subcommands {
  constructor(subparsers, command_dict) {
    super(subparsers, command_dict, 'data', [
...
class RobotStateCommands extends Subcommands {
  constructor(subparsers, command_dict) {
    super(subparsers, command_dict, 'state', [
      FullStateCommand,
...
class TimeSyncCommand extends Command {
  ...
  async _run(robot, options) {
    ...
    if (!(await endpoint.establish_timesync(undefined, true))) {
...
class BecomeEstopCommand extends Command {
  ...
  async _run(robot, options) {
    ...
    await endpoint.force_simple_setup();
...
class DataAcquisitionCommand extends Subcommands {
  constructor(subparsers, command_dict) {
    super(subparsers, command_dict, 'acquire', [
