...
class PayloadRegistrationClient extends BaseClient {
	...
	async register_payload(payload, secret, args){
		const req = new payload_registration_protos.RegisterPayloadRequest();
		req.setPayload(payload);
		if(secret) req.setPayloadSecret(secret);
		return await this.call(this._stub.registerPayload, request, null, _payload_registration_error, args);
	}

	register_payload_async(payload, secret, args){
		var req = new payload_registration_protos.RegisterPayloadRequest();
		req.setPayload(payload);
		if(secret) req.setPayloadSecret(secret);
		return this.call_async(this._stub.registerPayload, req, null, _payload_registration_error, args);
	}

	async update_payload_version(guid, secret, updated_version, args){
		...
		return await this.call(this._stub.updatePayloadVersion, req, null, _update_payload_version_error, args);
	}

	update_payload_version_async(guid, secret, updated_version, args){
		var req = new payload_registration_protos.UpdatePayloadVersionRequest();
	...
	async get_payload_auth_token(guid, secret, args){
		const req = new payload_registration_protos.GetPayloadAuthTokenRequest()
		.setPayloadGuid(guid)
		.setPayloadSecret(secret);
		return await this.call(this._stub.getPayloadAuthToken, req, _get_token, _get_payload_auth_token_error, args);
	}
...
const _UPDATE_PAYLOAD_VERSION_STATUS_TO_ERROR = {
	...
	STATUS_INVALID_CREDENTIALS: [InvalidPayloadCredentialsError, 'The payload credentials do not match any payload registered to the robot.']
}
...
function _payload_registration_error(response){
	return error_factory(response, response.status, Object.keys(payload_registration_protos.RegisterPayloadResponse.Status), _REGISTER_PAYLOAD_STATUS_TO_ERROR);
}

function _update_payload_version_error(response){
	return error_factory(response, response.status, Object.keys(payload_registration_protos.UpdatePayloadVersionResponse.Status), _UPDATE_PAYLOAD_VERSION_STATUS_TO_ERROR);
}

function _get_payload_auth_token_error(response){
	return error_factory(response, response.status, Object.keys(payload_registration_protos.GetPayloadAuthTokenResponse.Status), _GET_PAYLOAD_AUTH_TOKEN_STATUS_TO_ERROR);
}
...
class PayloadRegistrationKeepAlive {
	...
	constructor(pay_reg_client, payload, secret, registration_interval_secs = 30_000, logger = null, rpc_timeout_secs = null){
