...
function _error_from_response(response) {
  for (const image_response of response.getImageResponsesList()) {
    ...
    const result = error_factory(
      response,
      image_response.getStatus(),
      Object.keys(image_pb.ImageResponse.Status),
      _STATUS_TO_ERROR,
    );
...
function write_pgm_or_ppm(image_response, filename = '', filepath = './') {
  ...
  if (image_response.getShot().getImage().getPixelFormat() === image_pb.Image.PixelFormat.PIXEL_FORMAT_RGB_U8) {
    ...
    image_response.getShot().getImage().getPixelFormat() in
    [
      image_pb.Image.PixelFormat.PIXEL_FORMAT_GREYSCALE_U8,
      image_pb.Image.PixelFormat.PIXEL_FORMAT_DEPTH_U16,
      image_pb.Image.PixelFormat.PIXEL_FORMAT_GREYSCALE_U16,
    ]
  ) {
    num_channels = 1;
  } else {
    console.log(
      `[IMAGE] Unsupported pixel format for PGM/PPM: ${
        image_pb.Image.PixelFormat[image_response.getShot().getImage().getPixelFormat()]
      }.`,
    );
    return;
  }
  ...
  if (filename.length === 0) filename = `image-${image_response.getSource().getName()}${file_extension}`;
  ...
  console.log(
    `[IMAGE] Saved matrix with pixel values from camera "${image_response
      .getSource()
