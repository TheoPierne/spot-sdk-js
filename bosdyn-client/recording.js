...
const recording_service_pb = require('../bosdyn/api/graph_nav/recording_service_pb');
...
class GraphNavRecordingServiceClient extends BaseClient {
	...
	async start_recording(lease = null, recording_environment = null, args){
		const request = this._build_start_recording_request(lease, recording_environment);
		return await this.call(this._stub.startRecording, request, _get_status, _start_recording_error, args);
	}
	...
	async stop_recording(lease = null, args){
		const request = this._build_stop_recording_request(lease);
		return await this.call(this._stub.stopRecording, request, _get_status, _stop_recording_error, args);
	}
	...
	async get_record_status(args){
		const request = this._build_get_record_status_request();
		return await this.call(this._stub.getRecordStatus, request, _get_response, common_header_errors, args);
	}
	...
	async set_recording_environment(lease = null, recording_environment = null, args){
		const request = this._build_set_recording_environment_request(lease, recording_environment);
		return await this.call(this._stub.SetRecordingEnvironment, request, null, common_header_errors, args);
	}
	...
	async create_waypoint(lease = null, waypoint_name = null, recording_environment = null, args){
		const request = this._build_create_waypoint_request(waypoint_name, recording_environment, lease);
		return await this.call(this._stub.createWaypoint, request, _get_response, _create_waypoint_error, args);
	}
	...
	async create_edge(lease = null, edge = null, args){
		const request = this._build_create_edge_request(edge, lease);
		return await this.call(this._stub.createEdge, request, _get_status, _create_edge_error, args);
	}

	create_edge_async(lease = null, edge = null, args){
		var request = this._build_create_edge_request(edge, lease);
	...
	static make_waypoint_environment(name, region = WaypointRegion.DEFAULT_REGION, dist_2d = null, args){
		const waypoint_env = new map_pb.Waypoint.Annotations().setName(name);
		if(region == WaypointRegion.DEFAULT_REGION){
			const scan_match_region = new graph_nav.Waypoint.Annotations.LocalizeRegion()
			...
			const scan_match_region = new graph_nav.Waypoint.Annotations.LocalizeRegion()
			.setState(map_pb.AnnotationState.ANNOTATION_STATE_NONE);
			waypoint_env.setScanMatchRegion(scan_match_region);
		}
		return waypoint_env;
	}

	static make_edge_environment(vel_limit = null, direction_constraint = map_pb.Edge.Annotations.DirectionConstraint.DIRECTION_CONSTRAINT_NONE, require_alignment = false, flat_ground = false, ground_mu_hint = .8, grated_floor = false){
...
const _START_RECORDING_STATUS_TO_ERROR = {
	...
	STATUS_NOT_LOCALIZED_TO_EXISTING_MAP: [NotLocalizedToExistingMapError, 'The robot is not localized to the existing map and cannot start recording.'],
	...
	STATUS_REMOTE_CLOUD_FAILURE_NOT_IN_DIRECTORY: [RemoteCloudFailureNotInDirectoryError, 'Failed to start recording because a remote point cloud (e.g. a LIDAR) is not registered to the service directory.'],
	STATUS_REMOTE_CLOUD_FAILURE_NO_DATA: [RemoteCloudFailureNoDataError, 'Failed to start recording because a remote point cloud (e.g. a LIDAR) is not delivering data.'],
	STATUS_FIDUCIAL_POSE_NOT_OK: [FiducialPoseError, 'The pose of one or more required fiducials could not be determined accurately.']
};

const _STOP_RECORDING_STATUS_TO_ERROR = {
	STATUS_OK: [null, null],
	STATUS_NOT_LOCALIZED_TO_END: [NotLocalizedToEndError, 'Stop recording failed to localize to the last created waypoint.'],
	STATUS_NOT_READY_YET: [NotReadyYetError, 'The service is processing the map at it\'s current position. Try again in 1-2 seconds.']
};
...
const _CREATE_EDGE_STATUS_TO_ERROR = {
	...
	STATUS_MISSING_TRANSFORM: [EdgeMissingTransformError, 'The edge requested is missing the from_T_to transform in the edge.']
};

function _start_recording_error(response){
	return error_factory(response, response.getStatus(), Object.keys(recording_pb.StartRecordingResponse.Status), _START_RECORDING_STATUS_TO_ERROR);
}

function _stop_recording_error(response){
	return error_factory(response, response.getStatus(), Object.keys(recording_pb.StopRecordingResponse.Status), _STOP_RECORDING_STATUS_TO_ERROR);
}

function _create_waypoint_error(response){
	return error_factory(response, response.getStatus(), Object.keys(recording_pb.CreateWaypointResponse.Status), _CREATE_WAYPOINT_STATUS_TO_ERROR);
}

function _create_edge_error(response){
	return error_factory(response, response.getStatus(), Object.keys(recording_pb.CreateEdgeResponse.Status), _CREATE_EDGE_STATUS_TO_ERROR);
}
