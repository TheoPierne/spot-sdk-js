...
class SpotCheckClient extends BaseClient {
	...
	async spot_check_command(request, args){
		return await this.call(this._stub.spotCheckCommand, request, null, _spotcheck_command_error_from_response, args);
	}
	...
	async spot_check_feedback(request, args){
		return await this.call(this._stub.spotCheckFeedback, request, null, _spotcheck_feedback_error_from_response, args);
	}
	...
	async camera_calibration_command(request, args){
		return await this.call(this._stub.cameraCalibrationCommand, request, null, _calibration_command_error_from_response, args);
	}

	camera_calibration_command_async(request, args){
		return this.call_async(this._stub.cameraCalibrationCommand, request, null, _calibration_command_error_from_response, args);
	}

	async camera_calibration_feedback(request, args){
		return await this.call(this._stub.cameraCalibrationFeedback, request, null, _calibration_feedback_error_from_response, args);
	}

	camera_calibration_feedback_async(request, args){
		return this.call_async(this._stub.cameraCalibrationFeedback, request, null, _calibration_feedback_error_from_response, args);
	}
...
async function run_spot_check(spot_check_client, lease, timeout_sec = 120_000, update_frequency = 0.25, verbose = false){
	...
	while (Date.now() < end_time){
		...
		if (res.getstate() == spot_check_pb.SpotCheckFeedbackResponse.State.STATE_WAITING_FOR_COMMAND || res.getState() == spot_check_pb.SpotCheckFeedbackResponse.State.STATE_FINISHED){
			if (verbose) spot_check_client.logger.info("[SPOT CHECK] Spot check routine complete!");
			return res;
		}
		if (verbose) spot_check_client.logger.info(`[SPOT CHECK] Spot check ${Math.floor(res.getProgress() * 100)}% complete !`);
	}
	throw new SpotCheckTimedOutError();
}

async function run_camera_calibration(spot_check_client, lease, timeout_sec = 1200_000, update_frequency = 0.25, verbose = false){
	const start_time = Date.now();
	const end_time = start_time + timeout_sec;
	const update_time = 1.0 / update_frequency;
	...
	while (Date.now() < end_time){
		...
		if (res.getStatus() == spot_check_pb.CameraCalibrationFeedbackResponse.STATUS_PROCESSING){
			if (verbose) spot_check_client.logger.info(`[SPOT CHECK] Camera calibration ${Math.floor(res.getProgress() * 100)}% complete!`);
		}
	}
...
const _SC_ERROR_TO_ERROR = {
	...
	ERROR_UNEXPECTED_POWER_CHANGE: [SpotCheckUnexpectedPowerChangeError, 'Power error occurred while running spot check.'],
...
const _CAL_STATUS_TO_ERROR = {
	...
	STATUS_ROBOT_COMMAND_ERROR: [CameraCalibrationRobotCommandError, 'Robot command error occurred while running calibration.'],
...
function _spot_check_error_from_response(response){
	return error_factory(response, response.getError(), Object.keys(spot_check_pb.SpotCheckFeedbackResponse.Error), _SC_ERROR_TO_ERROR);
}
...
function _calibration_feedback_error_from_response(response){
	if(response.getStatus() == spot_check_pb.CameraCalibrationFeedbackResponse.Status.STATUS_LEASE_ERROR) return new LeaseUseError(response);
    return _cal_status_error_from_response(response);
}

function _cal_status_error_from_response(response){
	return error_factory(response, response.getStatus(), Object.keys(spot_check_pb.CameraCalibrationFeedbackResponse.Status), _CAL_STATUS_TO_ERROR);
}
