...
const {
	RobotTimeConverter, now_nsec, parse_timespan, 
	nsec_to_timestamp, set_timestamp_from_nsec, timestamp_to_nsec
...
class TimeSyncClient extends BaseClient {
	...
	get_time_sync_update_async(previous_round_trip, clock_identifier, args){
		var req = this.#get_time_sync_update_request(previous_round_trip, clock_identifier);
	...
	async _timesync_thread(){
		try{
			while(!this.should_exit){
				...
				if (!response || response.getState().getStatus() == time_sync_pb.TimeSyncState.Status.STATUS_MORE_SAMPLES_NEEDED){
					//pass
