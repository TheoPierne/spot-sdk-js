...
const { BaseClient, error_factory, error_pair, common_header_errors } = require('./common');
...
function _error_from_response(response) {
  for (const point_cloud_response of response.getPointCloudResponses()) {
    const result = error_factory(
      response,
      point_cloud_response.getStatus(),
      Object.keys(point_cloud_protos.PointCloudResponse.Status),
      _STATUS_TO_ERROR,
    );
 ...
 * @returns {point_cloud_protos.PointCloudRequest} The PointCloudRequest protobuf message for the given parameters.
 */
