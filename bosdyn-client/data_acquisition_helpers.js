...
async function issue_acquire_data_request(data_acq_client, acquisition_requests, group_name, action_name, metadata = null){
    const action_id = new data_acquisition_pb.CaptureActionId().setActionName(action_name).setGroupName(group_name);
    let request_id = null;

    try{
        request_id = await data_acq_client.acquire_data(acquisition_requests, action_name, action_id.getGroupName(), null, metadata);
    }catch(e){
...
async function acquire_and_process_request(data_acquisition_client, acquisition_requests, group_name, action_name, metadata = null, block_until_complete = true){
    const {request_id, action_id} = await issue_acquire_data_request(data_acquisition_client, acquisition_requests, group_name, action_name, metadata);
    ...
    while(true){
        ...
        console.log(`[DATA ACQUISITION HELPERS] Current status is: ${data_acquisition_pb.GetStatusResponse.Status[get_status_response.getStatus()]}`);
        if(get_status_response.getStatus() == data_acquisition_pb.GetStatusResponse.Status.STATUS_COMPLETE) return true;
        ...
        if(get_status_response.getStatus() == data_acquisition_pb.GetStatusResponse.Status.STATUS_REQUEST_ID_DOES_NOT_EXIST){
            console.log(`[DATA ACQUISITION HELPERS] The acquisition request id "${request_id}" is unknown: ${get_status_response}`);
            return false;
        }
        await sleep(200);
    }
...
async function cancel_acquisition_request(data_acq_client, request_id){
    ...
    try{
        ...
        console.log(`[DATA ACQUISITION HELPERS] Status of the request to cancel the data-acquisition in progress: ${data_acquisition_pb.CancelAcquisitionResponse.Status[is_cancelled_response.getStatus()]}`);
    }catch(e){
    ...
    while(true){
        ...
        console.log(`[DATA ACQUISITION HELPERS] Request ${request_id} status: ${data_acquisition_pb.GetStatusResponse.Status[get_status_response.getStatus()]}`);
...
function make_time_query_params(start_time_secs, end_time_secs, robot){
    ...
    const time_range = new data_acquisition_store_pb.TimeRangeQuery().setFromTimestamp(from_timestamp).setToTimestamp(to_timestamp);
...
async function make_time_query_params_from_group_name(group_name, data_store_client){
    ...
    try{
        ...
        console.error(`[DATA ACQUISITION HELPERS] Failed to list the capture action ids for group_name ${group_name}: ${e}`);
        return null;
    }

    let start_time = [null, null];
    let end_time = [null, null];
    for(const action_id of saved_capture_actions){
        const timestamp = action_id.getTimestamp();
    ...
    const time_range = new data_acquisition_store_pb.TimeRangeQuery().setFromTimestamp(start_time[1]).setToTimestamp(end_time[1]);
...
function download_data_REST(query_params, hostname, token, destination_folder = './', additional_params = null){
    ...
    try{
        ...
        fetch(url(`https://${hostname}/v1/data-buffer/daq-data/`, get_params), options)
        .then(res => res.buffer())
        .then(res => {
            ...
            if(res.status == 204){
                const timeGetFrom = query_params.getTimeRange().getFromTimestamp();
                const timeGetTo = query_params.getTimeRange().getToTimestamp();
                console.log(`[DATA ACQUISITION HELPERS] No content available for the specified download time range (in seconds): [${(timeGetFrom.getSeconds() * 1e9 + timeGetFrom.getNanos()) / 1e9}, ${(timeGetTo.getSeconds() * 1e9 + timeGetTo.getNanos()) / 1e9}]`);
                return false;
            }
            ...
            writeFileSync(download_file, res);
        }).catch(e => {
            console.error(e);
            return false;
        });
