...
function _process_topology_common_errors(response) {
  if (response.getStatus() === map_processing_pb.ProcessTopologyResponse.Status.STATUS_INVALID_GRAPH) {
    ...
    response.getStatus() === map_processing_pb.ProcessTopologyResponse.Status.STATUS_MISSING_WAYPOINT_SNAPSHOTS
  ) {
    return new MissingSnapshotsError('The uploaded map has missing waypoint snapshots.');
  } else if (
    response.getStatus() === map_processing_pb.ProcessTopologyResponse.Status.STATUS_MAP_MODIFIED_DURING_PROCESSING
  ) {
...
const __ANCHORING_COMMON_ERRORS = {
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_MISSING_WAYPOINT_SNAPSHOTS]: [
    MissingSnapshotsError,
    'The uploaded map has missing waypoint snapshots.',
  ],
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_OPTIMIZATION_FAILURE]: [
    OptimizationFailureError,
    'The anchoring optimization failed.',
  ],
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_INVALID_GRAPH]: [
    InvalidGraphError,
    'The graph is invalid topologically, for example containing missing waypoints referenced by edges.',
  ],
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_INVALID_PARAMS]: [
    InvalidParamsError,
    'The parameters passed to the optimizer do not make sense (e.g negative weights).',
  ],
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_CONSTRAINT_VIOLATION]: [
    ConstraintViolationError,
    'One or more anchors were moved outside of the desired constraints.',
  ],
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_MAX_ITERATIONS]: [
    MaxIterationsError,
    'The optimizer reached the maximum number of iterations before converging.',
  ],
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_MAX_TIME]: [
    MaxTimeError,
    'The optimizer timed out before converging.',
  ],
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_INVALID_HINTS]: [
    InvalidHintsError,
    'One or more of the hints passed in to the optimizer are invalid (do not correspond to real waypoints or objects).',
  ],
  [map_processing_pb.ProcessAnchoringResponse.Status.STATUS_MAP_MODIFIED_DURING_PROCESSING]: [
    MapModifiedError,
    'The map was modified on the server by another client during processing. Please try again.',
  ],
};
...
class MapProcessingServiceClient extends BaseClient {
  ...
  process_topology(params, modify_map_on_server, args) {
    const request = this._build_process_topology_request(params, modify_map_on_server);
    return this.call(
      this._stub.processTopology,
      request,
      _get_streamed_topology_response,
      _process_topology_streamed_errors,
      args,
    );
  }

  process_anchoring(params, modify_anchoring_on_server, stream_intermediate_results, initial_hint = null, args) {
    const request = this._build_process_anchoring_request(
      params,
    ...
    return this.call(
      ...
      request,
      _get_streamed_anchoring_response,
      _process_anchoring_streamed_errors,
      args,
    );
  }
}
