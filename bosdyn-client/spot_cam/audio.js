...
class AudioClient extends BaseClient {
  ...
  list_sounds(args) {
    ...
    return this.call(
      ...
      request,
      this._list_sounds_from_response,
      this._audio_error_from_response,
      args,
    );
  }
  ...
  set_volume(percentage, args) {
    const request = new audio_pb.SetVolumeRequest().setVolume(percentage);
    return this.call(
      this._stub.setVolume,
      request,
      this._set_volume_from_response,
      this._audio_error_from_response,
      args,
    );
  }
  ...
  get_volume(args) {
    ...
    return this.call(
      this._stub.getVolume,
      request,
      this._get_volume_from_response,
      this._audio_error_from_response,
      args,
    );
  }
  ...
  play_sound(sound, gain = null, args) {
    let request;
    if (gain) {
    ...
    return this.call(
      this._stub.playSound,
      request,
      this._play_sound_from_response,
      this._audio_error_from_response,
      args,
    );
  }
  ...
  load_sound(sound, data, max_chunk_size = 1024 * 1024, args) {
    function* yield_requests(dataToYield) {
      const chunk = new data_chunk_pb.DataChunk().setTotalSize(dataToYield.length);
    ...
    return this.call(
      ...
      yield_requests(data),
      this._load_sound_from_response,
      this._audio_error_from_response,
      args,
    );
  }

  set_audio_capture_channel(channel, args) {
    const request = new audio_pb.SetAudioCaptureChannelRequest().setChannel(channel);
    return this.call(this._stub.setAudioCaptureChannel, request, null, this._audio_error_from_response, args);
  }
  ...
  set_audio_capture_gain(channel, gain, args) {
    const request = new audio_pb.SetAudioCaptureGainRequest().setChannel(channel).setGain(gain);
    return this.call(this._stub.setAudioCaptureGain, request, null, this._audio_error_from_response, args);
  }
